...
if (Prototype.BrowserFeatures.XPath) {
  document._getElementsByXPath = function(expression, parentElement) {
    var results = [];
    var query = document.evaluate(expression, window.$p(parentElement) || document,
...
Element.Methods = {
  ...
  immediateDescendants: function(element) {
    var results = [], child = window.$p(element).firstChild;
    while (child) {
      if (child.nodeType === 1) {
  ...
  up: function(element, expression, index) {
    element = window.$p(element);
    if (arguments.length == 1) return window.$p(element.parentNode);
  ...
  identify: function(element) {
    ...
    if (id) return id;
    do { id = 'anonymous_element_' + Element.idCounter++ } while (window.$p(id));
  ...
  hasClassName: function(element, className) {
    if (!(element = window.$p(element))) return;
  ...
  descendantOf: function(element, ancestor) {
    element = window.$p(element), ancestor = window.$p(ancestor);
...
(function() {
  ...
  Element.Layout = Class.create(Hash, {
    ...
    _begin: function() {
      ...
      if (width && (positionedWidth === width)) {
        ...
        var parent = element.parentNode, pLayout = window.$p(parent).getLayout();
  ...
  function getOffsetParent(element) {
    ...
    if (!isInline && element.offsetParent) return window.$p(element.offsetParent);
  ...
  serializeElements: function(elements, options) {
    ...
    return elements.inject(initial, function(result, element) {
      if (!element.disabled && element.name) {
        key = element.name; value = window.$p(element).getValue();
...
Form.Methods = {
  ...
  getElements: function(form) {
    var elements = window.$p(form).getElementsByTagName('*'),
        element,
        arr = [ ],
  ...
  request: function(form, options) {
    form = window.$p(form), options = Object.clone(options || { });
...
Form.Element.Methods = {
  ...
  disable: function(element) {
    ...
    if (element.type === 'file') {
      var deleteUploadBtns = $window.$p('#' + element.closest('li').id + ' .qq-upload-delete');
